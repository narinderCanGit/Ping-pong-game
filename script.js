...
function resetBoard(rodName) {
  ...
  if (rodName === rod2Name) {
    ...
    //console.log(`top:${rod2.offsetTop}, height:${window.innerHeight}`);
    ballSpeedY = -2;
  }
...
window.addEventListener("keypress", function () {
  ...
  if (event.code === "KeyD" && rodRect.x + rodRect.width < window.innerWidth) {
    // console.log(`D-x:${rodRect.x}, width:${rodRect.width}, innerWidth: ${window.innerWidth}`);
    rod1.style.left = rodRect.x + rodSpeed + "px"; // rod speed is movement of rod in px in one click
    rod2.style.left = rod1.style.left; //rod 1 and rod 2 move together
  ...
  if (event.code === "Enter") {
    if (!gameOn) {
      ...
      // console.log(`${ballRect.y}, BallDia: ${ballRect.width}`);
      ...
      movement = setInterval(function () {
        ...
        let ballPos = ballX + ballDia / 2; //ball + half of BallDia
        //console.log(`ballPos: ${ballPos}, ballX: ${ballX}`);
        ...
        if (ballY <= rod1Height) {
          ...
          //console.log(`ballPos: ${ballPos}, rod1x: ${rod1X}`);
